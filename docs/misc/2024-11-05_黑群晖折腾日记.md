...
2025 年 11 月 29 日，电网维护临时停电。等当晚来电时，我在学校（异地）发现 NAS 部署的服务全挂了。

遂打视频电话联系家里人，远程看一下设备状态，发现磁盘异常安静，硬盘灯未亮，只亮电源指示灯。

于是进入路由器后台，发现一个没见过的 IP 设备，此外之前 DSM 的 IP 已不见，局域网下另一台设备访问新 IP 进入了 RR 引导界面。

结合上网搜寻，推测大概率是**引导**出问题了。于是下单了一个海康威视的 8GB U盘，重新下载最新版本的引导、烧录、替换旧 U盘、开机、进入 RR 引导。提示从硬盘中发现引导配置文件，于是选择从硬盘中恢复引导配置文件，重新编译，启动。成功启动！但是日志提示“外部 USB 存储挂载失败”，我没管。

这一次异常的顺利，以为没问题了，我远程关机之后，我家人装上机箱外壳，我重新启动，发现启动失败！查路由器后台，又进入了引导配置界面。难道是引导刚启动就损坏了？难道跟那条日志有关联？

于是拔下海康 U 盘，拿到另一台电脑重新烧录了一下，想重走成功过的旧路。这次尝试多拔了一块硬盘，因为 AI 告诉我引导做好之前可以拔掉硬盘，防止开关机损坏硬盘。局域网访问 RR 配置页面拒绝访问，插上显示器无 BIOS 画面。由于远程操作，无法物理排查，又赶上期末周，便放弃抢救。

2025 年 12 月 31 日，趁元旦假期上午回到了家中，吃过午饭下午立即开始排查问题，这次又买了一个铠侠的 U 盘当作引导盘。

显示器没画面让我一头雾水，把内存条、CPU、纽扣电池都插拔了一次，显示器也换了一个，关开几次还是“啥都亮没图像”。后来开始怀疑是不是 HDMI 线的问题啊，把这跟 HDMI 线换到别的电脑上，还是没有图像。确诊！换 HDMI 线！开机，图像来了！

多次开关机排查问题，就没插硬盘，引导固件版本是重新下载的**与之前一致的版本**。此次在 RR 配置页面，多设置了”直接启动“，会导致开机时引导两次。一根显示器数据线耽误了俩小时，乱了手脚。

宕机抢救回来，由于前面开机只插了一块硬盘，导致我的双盘位 RAID1 数据不同步，提示存储空间已降级。

趁这个机会，我把 RAID1 降级为了 BASIC。在写这篇文章的同时，NAS 正在执行资料转移。
